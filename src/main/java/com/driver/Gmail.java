...
public class Gmail extends Email {
    ...
    public void receiveMail(Date date, String sender, String message){
        ...
        if(inbox.size() == inboxCapacity){
            Triple<Date , String, String> oldestmsg = inbox.get(0);
            inbox.remove(0);
            trash.add(oldestmsg);

        }else{
            Triple<Date,String,String> newmsg = Triple.of(date,sender,message);
            inbox.add(newmsg);
        }
    }
    ...
    public void deleteMail(String message){
        ...
        for(int i = 0 ; i < inbox.size(); i++){
            String ele = inbox.get(i).getRight();
            if(message.equals(ele)){
                idx = i;
                break;
            }
        }
